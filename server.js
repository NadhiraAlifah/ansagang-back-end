...
app.post('/api/technologies-items', upload.single('img'), (req, res) => { new technologiesItemsModel({ id: req.body.id, title: req.body.title, img: req.file.path }).save(err => { if (!err) res.json({ success: true }); else res.json({ success: false, error: err }) }) })
